...
CONTEXT = {
    ...
    "prior_attempts_summary": (
        "We already sent an SMS with an upload link on August 18, and two "
        "reminder emails on August 19 and August 20. None have been opened yet."
    ),
    "callback_number": "+91-22-4000-1234",
}

# Retell's general_prompt supports {{variable}} templating against
# retell_llm_dynamic_variables passed at call time (or default_dynamic_variables
# set on the LLM itself). Keep the prompt generic/reusable; only CONTEXT changes
# per call.
GENERAL_PROMPT = """
You are an automated voice assistant calling on behalf of Bezzit, a real
estate transaction platform. You are NOT a human. Say so explicitly, ideally
...
you the upload link now" or "I've noted you'd like a callback"). Then use the
end_call tool.
""".strip()

BEGIN_MESSAGE = (
    "Hi, this is an automated assistant calling from Bezzit -- is this "
    "{{customer_name}}? This call is being recorded."
)
